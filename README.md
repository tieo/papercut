...
Pre-alpha. Repo scaffolding + core interfaces. No working model yet.

## Approach

This is the field known as **Page Stream Segmentation (PSS)** in the literature. The published SOTA on the public TABME++ benchmark is ~80% STP using a fine-tuned 7B LLM — impressive but not CPU-friendly. For our constraints we follow the cheaper, language-agnostic direction:

- Per-page representation = vision embedding (DiT or small CNN) ⊕ multilingual text embedding (MiniLM / mE5).
...
```
src/papercut/
├── data/        # dataset registry + per-source downloaders
...
├── eval/        # metrics (page-F1, doc-F1, Panoptic Quality, STP, MNDD) + prospective harness
└── serve/       # paperless-ngx integration (consume-folder watcher + pre-consume hook)
```
